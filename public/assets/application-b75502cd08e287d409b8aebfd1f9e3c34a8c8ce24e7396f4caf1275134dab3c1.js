...
!(function (context, definition) {
  if (typeof define == 'function' && typeof define.amd  == 'object') define(['jquery'], definition);
    ...
    var methods = {
        ...
        count_one: function(start_value) {
            return this.each(function() {
                var e = $(this);
                var data = e.data('counter');
                var start_nums = split(start_value, /([^0-9]+)/); 
                if (data.intervalId) {
                  clearTimeout(data.intervalId);
                }
                $.each(data.parts, function(pindex, pvalue){
                    pvalue.value = parseInt(start_nums[start_nums.length - data.format.length + pindex] || 0, 10); ;
                });
                refresh($(this), 0);
                data.intervalId = setTimeout($.proxy(one_tick, this), data.interval);
            });
        },
  ...
  App.vote_result = App.cable.subscriptions.create("VoteResultChannel", {
    ...
    received: function(data) {
      ...
      if (window.counter_ready) {
        ...
        if (typeof white_counter !== 'undefined') {
          new_val = data["white"];
        ...
        if (typeof voter_ticket_id !== 'undefined') {
          return this.update_voter_msg(voter_ticket_id, voter_team);
        }
      } else {
        console.log("counter isn't ready");
        return window.hang_data = data;
      }
    },
    update_voter_msg: function(voter_ticket_id, voter_team) {
      var update_div;
      if (typeof this.red_msg_array !== 'object') {
        this.red_msg_array = [];
      }
      if (typeof this.white_msg_array !== 'object') {
        this.white_msg_array = [];
      }
      update_div = $(".counter-group." + voter_team + " .voter");
      if (voter_team === "red") {
        return this.update_voter_msg_to_array(this.red_msg_array, voter_ticket_id, $(".counter-group.red .voter"));
      } else {
        return this.update_voter_msg_to_array(this.white_msg_array, voter_ticket_id, $(".counter-group.white .voter"));
      }
    },
    update_voter_msg_to_array: function(array, voter_ticket_id, voter_div) {
      var line;
      if (array.length >= 8) {
    ...
    get_result: function() {
      ...
      return this.perform('get_result');
    },
    sleep: function(ms) {
      var results, start;
...
(function() {
  $(window).load(function() {
    var counter_name, i, len, ref;
    ref = ["#red-counter", "#white-counter"];
    for (i = 0, len = ref.length; i < len; i++) {
      counter_name = ref[i];
    ...
    if (window.hang_data) {
      ...
      return App.vote_result.received(window.hang_data);
    }
  });
